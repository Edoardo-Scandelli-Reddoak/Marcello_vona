...
export default function HomeClient({ initialHero }: { initialHero: HeroSettings | null }) {
  ...
  // Hero già risolto lato server (SSR) e passato come prop: così l'immagine
  // corretta è nel primo paint e non c'è più il "flash" dell'immagine vecchia.
  const [hero, setHero] = useState<HeroSettings | null>(initialHero);
  ...
  useEffect(() => {
    ...
    // Fetch hero lato client solo come fallback se il fetch SSR non è
    // riuscito (es. rete interna Docker in dev). In produzione initialHero
    // è già valorizzato, quindi qui non si ri-fetcha e non c'è swap visibile.
    if (!initialHero) {
